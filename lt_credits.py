...
def get_lt_credits(df_rmas):
    ...
    if resp.status_code == 200:
        ...
        df_credits = pd.DataFrame.from_dict(
            credit_dict,
            orient="index",
            columns=[
                "product_variant_id",
        ...
        output = (
            "Number of approved LeafTrade credits grabbed: "
            + str(len(df_credits.index))
            + "\n"
        )
        print_gui(output)
...
def get_product_variants(df_credits):
    ...
    for variant in enumerate(df_credits.values):
        ...
        try:
            ...
            print_gui(
                f"{resp.status_code} - Error requesting LeafTrade Credit for Variant {v_id}.",
                "\n",
            )
        ...
        if resp.status_code == 200:
            ...
            variant_dict[variant_id] = [variant_ndc]

    df_variant = pd.DataFrame.from_dict(
        variant_dict, orient="index", columns=["variant_ndc"]
    )
...
def get_EPICOR_skus():
    ...
    df_sku = pd.DataFrame(
        sku_list,
        columns=["PRODUCT NAME", "PartDesc", "LOT #", "LotDesc", "Batch", "NDC"],
    )
...
def compare_LT_EPICOR(DF_LT_CREDITS, DF_EPICOR_SKU, DF_CUSTOMERS):
    ...
    for cust in unique_customers:
        ...
        df_final.to_csv(
            f"{ROOT_CUSTOMER_ORDERS_FOLDER}\\{today} Credit Form -{disp}_{loc_ID}.csv"
        )
        rma_arr.append(df_final)

    print_gui(
        f"Epicor and Leaf Trade Credits Matched Up. Files saved in {ROOT_CUSTOMER_ORDERS_FOLDER}\n"
    )
    return rma_arr


def create_epicor_rma(custNum):
    EPI_REST_RMA_ENDPOINT = (
        "https://SERVERNAME/DBINSTANCE/api/v1/Erp.BO.RMAProcSvc/RMAProcs"
    )
...
def create_epicor_rma_detail(
    rmaNum, rmaLine, part, partDesc, unitPrice, qty, lotNum, notes, reason, credit_id
):
    EPI_REST_RMADTL_ENDPOINT = (
        "https://SERVERNAME/DBINSTANCE/api/v1/Erp.BO.RMAProcSvc/RMADtls"
    )
...
def create_epicor_rma_detail_credit(rmaNum, rmaLine):
    EPI_REST_RMADTLCREDIT_ENDPOINT = (
        "https://SERVERNAME/DBINSTANCE/api/v1/Erp.BO.RMAProcSvc/RMACreditAdd"
    )
    ...
    if resp.status_code == 200:
        ...
        print_gui(
            f"Successful EPICOR REST Request - Invoice {invoice_num} Ln {invoice_line} Created"
        )

        return [invoice_num, invoice_line]


def upcate_epicor_rma_detail_credit(
    invoiceNum, invoiceLn, unitPrice, custNum, qty, lot
):
    EPI_REST_ARINVOICE_ENDPOINT = f"https://SERVERNAME/DBINSTANCE/api/v1/Erp.BO.ARInvoiceSvc/InvcDtls(CP,{invoiceNum},{invoiceLn})"
    ...
    if resp.status_code == 204:
        print_gui(
            f"Successful EPICOR REST Request - Invoice {invoiceNum} Ln {invoiceLn} Updated"
        )
...
def process_epicor_rmas(rma):
    ...
    for i, ln in rma.iterrows():
        ...
        upcate_epicor_rma_detail_credit(
            invoice_num, invoice_ln, unitPrice, custNum, qty, lotNum
        )
...
def fullRun():
    ...
    parsed_credits_arr = compare_LT_EPICOR(
        df_lt_product_variants, df_epicor_skus, df_epicor_customers
    )
    with ThreadPoolExecutor(max_workers=72) as executor:
        results = executor.map(process_epicor_rmas, parsed_credits_arr)
