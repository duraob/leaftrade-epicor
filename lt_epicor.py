...
ROOT_CUSTOMER_ORDERS_FOLDER = r'\\svpo0fs01\Departmental\Customer Orders'
CURRENT_YEAR_FOLDER = os.path.join(ROOT_CUSTOMER_ORDERS_FOLDER, str(datetime.now().year))
...
def get_lt_orders():
    ...
    LT_ORDERS_ENDPOINT = f'/api/v3/vendor/orders/?status={STATUS}&include_order_items=true'
...
##        print_gui('Succesful Leaf Trade Web Request')
    ...
    ######### for each order, get product info, qty, unit price - split into objects in dict
        ...
        for order in orders_arr:
            ...
            df_order = pd.DataFrame.from_dict(orders_dict, orient='index', columns=['product_name', 'UNIT $', 'QTY', 'order_id', 'order_status', 'created_at', 'user_email', 'order_total', 'dispensary', 'lic', 'location'])
...
def get_lt_order(orderNum):
    BASE_URL = 'https://app.leaf.trade/'
    LT_ORDERS_ENDPOINT = f'/api/v3/vendor/orders/?invoice_id={orderNum}&include_order_items=true'
...
##    print_gui(f'Fetching SKUs from Epicor.\n')
    ...
    df_sku = pd.DataFrame(sku_list, columns=['PRODUCT NAME', 'PartDesc', 'TYPE', 'LOT #', 'LotDesc', 'Batch', 'NDC', 'EXP DATE', 'REG #', 'THC', 'BRAND NAME', 'GENETICS', 'WEIGHT / FORM'])
...
def compare_LT_EPICOR(LT_ORDERS_LIST, DF_EPICOR_SKU, DF_CUSTOMERS):
    ...
    for i,df in enumerate(LT_ORDERS_LIST):
        ...
        df_final = df_merge[['TYPE', 'EXP DATE', 'GENETICS', 'BRAND NAME', 'REG #', 'WEIGHT / FORM', 'QTY', 'UNIT $', 'THC', 'PRODUCT NAME', 'LOT #', 'dispensary', 'lic', 'location', 'PartDesc', 'order_id']]
        df_final.set_index('TYPE', inplace=True)
        df_final['TOTAL'] = df_final['QTY'] * df_final['UNIT $']
        total_col = df_final.pop('TOTAL')
        ####### fill out the rest of the info required to make a sales order ( should be on notebook of what they currently do )
...
##        df_final.to_csv(f'C:\\Users\\ben.durao\\Documents\\{today} CTPharma Order Form-{disp}_{loc}.csv')
        ##df_final.to_csv(f'{CURRENT_DAY_ORDERS_FOLDER}\\{today} CTPharma Order Form -{disp}_{loc_ID}.csv')

    print_gui(f'Epicor and Leaf Trade Orders Matched Up. Files saved in {CURRENT_DAY_ORDERS_FOLDER}\n')
    return df_final_arr

def create_epicor_order(custNum, poNum):
    EPI_REST_ORDERS_ENDPOINT = 'https://epicor10/ERP10Live/api/v1/Erp.BO.SalesOrderSvc/SalesOrders'
    ...
    resp = requests.post(EPI_REST_ORDERS_ENDPOINT, json=data, headers=headers, verify=False, auth=(EPI_USER,EPI_PWD)) #'O:\ERP\Projects\BDurao\LeafTrade APi\E10REST.cer'
...
def create_epicor_order_detail(orderNum, orderLine, part, partDesc, unitPrice, qty):
    EPI_REST_ORDERS_ENDPOINT = 'https://epicor10/ERP10Live/api/v1/Erp.BO.SalesOrderSvc/OrderDtls'
...
def create_epicor_order_rels(orderNum, orderLine, lot):
    EPI_REST_ORDERS_ENDPOINT = f'https://epicor10/ERP10Live/api/v1/Erp.BO.SalesOrderSvc/OrderRels(CP,{orderNum},{orderLine},1)'
    ...
    requests.patch(EPI_REST_ORDERS_ENDPOINT, json=data, headers=headers, verify=False, auth=(EPI_USER,EPI_PWD)) #'O:\ERP\Projects\BDurao\LeafTrade APi\E10REST.cer'
...
def process_epicor_orders(order):
    ...
    df = pd.DataFrame.from_dict(dict_order, orient='index', columns=['poNum', 'orderNum', 'disp', 'loc', 'date'])
...
def fullRun():
    ...
    parsed_orders_arr = compare_LT_EPICOR(lt_orders_arr, df_epicor_skus, df_epicor_customers)
    with ThreadPoolExecutor(max_workers=36) as executor:
        results = executor.map(
                process_epicor_orders,
                parsed_orders_arr
        )
    end = datetime.now()
    output = '\nProgram End: '+ str(end)
...
def oneRun(orderNo):
    ...
    if not orderNum.isnumeric():
        print_gui('ERROR: LeafTrade Orders are numeric, please give an appropriate entry.\n')
        return
