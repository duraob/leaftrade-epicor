...
ROOT_CUSTOMER_ORDERS_FOLDER = r"FILESHARE PATH"
CURRENT_YEAR_FOLDER = os.path.join(
    ROOT_CUSTOMER_ORDERS_FOLDER, (str(year) + " " + "Order Forms")
)
    ...
    if resp.status_code == 200:
        ...
        for product in products_arr:
            ...
            for stock in product["stock"]:
                ...
                if qty > 0:
                    product_dict[pull_num] = [pro_lot, qty, price, product_id, pull_num]

        df_pros = pd.DataFrame.from_dict(
            product_dict,
            orient="index",
            columns=["Lot", "Qty - LeafTrade", "Price", "product_id", "pull_num"],
        )
...
def get_lt_active_batches(df_products):
    ...
    for i, product in df_products.iterrows():
        ...
        LT_VARIANTS_ENDPOINT = (
            f"/api/v3/vendor/product-variants/?status=enabled&product_id={pro_id}"
        )
        ...
        if resp.status_code == 200:
            ...
            for pro in variant_arr:
                if (
                    pro["status_display"] == "Visible"
                    and product["pull_num"] == pro["stock"][0]["pull_number"]
                ):
    ...
    df_active_pros = pd.DataFrame.from_dict(
        active_products_dict,
        orient="index",
        columns=["Lot", "Qty - LeafTrade", "Price"],
    )
...
def compare_LT_EPICOR(DF_INVENTORY, DF_EPICOR_SKU):
    df_merge = DF_INVENTORY.merge(
        DF_EPICOR_SKU, how="inner", left_on="NDC", right_on="NDC"
    )
    df_final = df_merge[
        [
            "Sale Group",
