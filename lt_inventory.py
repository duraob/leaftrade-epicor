...
ROOT_CUSTOMER_ORDERS_FOLDER = r"FILESHARE FOLDER PATH"
...
def get_lt_inventory():
    ...
    if resp.status_code == 200:
        ...
        output = (
            "Number of LeafTrade inventory items grabbed: "
            + str(len(df_pros.index))
            + "\n"
        )
        print(output)
        return df_pros
...
def compare_LT_EPICOR(DF_INVENTORY, DF_EPICOR_SKU):
    df_merge = DF_INVENTORY.merge(
        DF_EPICOR_SKU, how="left", left_on="NDC", right_on="NDC"
    )
    df_final = df_merge[
        [
            "product_id",
    ...
    df_final = df_final.astype(
        {
            "Qty - LeafTrade": "int",
            "Qty Allocated - LeafTrade": "int",
            "Qty - Epicor": "int",
        }
    )
    df_final["Ending Balance"] = (
        df_final["Qty - Epicor"] - df_final["Qty Allocated - LeafTrade"]
    )
    df_final.sort_values(
        by=["Qty Allocated - LeafTrade", "Sale Group - Epicor"],
        ascending=False,
        inplace=True,
    )
    ...
    print_gui(
        f"Epicor and Leaf Trade Inventory Matched Up. Files saved in {ROOT_CUSTOMER_ORDERS_FOLDER}\n"
    )
