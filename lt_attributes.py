...
ROOT_CUSTOMER_ORDERS_FOLDER = r'\\svpo0fs01\Departmental\Customer Orders\LeafTrade Files\Attributes'
...
def get_lt_batches():
    ...
    ######### for each batch, get proptiers
        ...
        df_batch = pd.DataFrame.from_dict(batches_dict, orient='index', columns=['batch_ref', 'thc', 'thca', 'cbd', 'enabled'])
        df_batch.index.name = 'batch_id'
        df_batch.to_csv(f'{ROOT_CUSTOMER_ORDERS_FOLDER}\\batches_{today}.csv')

        output = 'Number of LeafTrade batches grabbed: ' + str(len(df_batch.index)) + '\n'
        print_gui(output)
        return df_batch
...
def get_lt_inventory():
    BASE_URL = 'https://app.leaf.trade/'
    LT_ORDERS_ENDPOINT = f'/api/v3/vendor/inventory/?page_size=10000&status=enabled&ordering=batch_ref'
    ...
    ######### for each batch, get proptiers
        ...
        df_inv = pd.DataFrame.from_dict(inv_dict, orient='index', columns=['batch_ref', 'pack_size', 'pack_uom', 'price', 'quantity', 'storefront_name'])
...
def patch_inventory(inv_id, batch_ref, pack_size, pack_uom, price, quantity, storefront_name):
    BASE_URL = 'https://app.leaf.trade/'
    ...
    """ % (batch_ref, pack_size, pack_uom, price, quantity, storefront_name)
    try:
...
def patch_inv_list(FILE_PATH):
    ...
    for i, inv in df_inv.iterrows():
        ...
        try:
            patch_inventory(inv_id, batch_ref, pack_size, pack_uom, price, quantity, storefront_name)
            print_gui('Succesful batch update.')
        except:
            print_gui(f'Failed batch update.')
