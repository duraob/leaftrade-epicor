    ...
    if request.method == "POST":
        if "migrate-all-orders" in request.form:
            ...
            try:
                ...
                alert = "Success - LeafTrade Allocations matched to Epicor Inventory in FILE SHARE."
            except:
                alert = "Error - Could not complete request to verify inventory."
        elif "pull-order-form" in request.form:
            try:
                lt_order_form.fullRun()
                alert = "Success - LeafTrade Order Form saved in FILE SHARE"
            except:
    ...
    if request.method == "POST":
        if "batchDL" in request.form:
            try:
                ...
                alert = (
                    "Error - Could not complete request to download LeafTrade batches."
                )
